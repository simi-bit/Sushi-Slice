...
public:
	MainMenu(sf::RenderWindow* menu, sf::Sprite* mainMenuBackground, std::vector<sf::Sprite>* mainMenuBackgroundLeft, std::vector<sf::Sprite>* mainMenuBackgroundRight,
		sf::Texture* textureMainMenuBackgroundLeft, sf::Texture* textureMainMenuBackgroundRight, sf::Text* startBtn, sf::Text* exitBtn, GameState* currentState) {
		// Screen Resolution
	...
	void display() {
		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
			if (startBtn->getGlobalBounds().contains((sf::Vector2f)menu->mapCoordsToPixel(sf::Vector2f(sf::Mouse::getPosition().x, sf::Mouse::getPosition().y)))) {
				*currentState = GameState::Playing;
			}
			else if (exitBtn->getGlobalBounds().contains((sf::Vector2f)menu->mapCoordsToPixel(sf::Vector2f(sf::Mouse::getPosition().x, sf::Mouse::getPosition().y)))) {
				menu->close();
			}
		}
	}
